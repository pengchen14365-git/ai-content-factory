...
def generate_summary(chapter_number, title):
    """为每个章节生成总结文案"""
    summaries = {
        1: {
            'title': 'OpenClaw简介与个人CRM',
    ...
    return summaries.get(chapter_number, {
        'title': title,
        'content': f"介绍{title}的核心概念和实际应用。",
        'platforms': {
            '小红书': f'{title}AI工具教程',
            '知乎': f'{title}详细解析',
            '抖音': f'{title}快速入门'
...
def create_final_report():
    ...
    report_dir = os.path.join(output_dir, f"openclaw_final_report_{timestamp}")
    ...
    with open(readme_path, 'w', encoding='utf-8') as f:
        f.write("# 🎬 OpenClaw 教程章节剪辑完整报告\n\n")
        f.write(f"**视频**: 21 INSANE Use Cases For OpenClaw\n")
        f.write(f"**总时长**: 33:44\n")
        f.write(f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**配置**: 所有 12 个章节 + 中英双语字幕 + 总结文案\n\n")
        ...
        for i, chapter in enumerate(chapters, 1):
            summary = generate_summary(i, chapter['title'])
            ...
            with open(chapter_path, 'w', encoding='utf-8') as chapter_f:
                ...
                chapter_f.write(f"**时间范围**: {chapter['start']}s - {chapter['end']}s\n")
                ...
                chapter_f.write(f"**视频片段**: `{safe_title}_clip.mp4`\n")
                chapter_f.write(f"**英文字幕**: `{safe_title}_original.srt`\n")
                chapter_f.write(f"**总结文案**: `{safe_title}_summary.md`\n\n")
            ...
            f.write(f"- **时长**: {duration} 秒 ({minutes}:{seconds:02d})\n")
        ...
        f.write("- ✅ 提供 12 个独立章节，可单独或组合发布\n\n")

        f.write("### 字幕格式\n\n")
        f.write("- ✅ **原文字幕**: SRT格式，时间码精确同步\n")
        f.write("- ✅ **双语字幕**: 中文+英文，位置对齐（需手动添加）\n")
        f.write("- ✅ **硬字幕选项**: 可直接烧录到视频上\n\n")

        f.write("### 文件结构\n\n")
        f.write("```\n")
        f.write(f"📁 openclaw_final_report_{timestamp}/\n")
        f.write("├── 📄 README.md                           # 主报告文件\n")
        f.write("├── 📁 chapters/                           # 章节详细报告 (12个)\n")
        f.write("└── 📁 video_files/                        # 视频文件目录\n")
        f.write("    ├── 📹 chapter_1_*.*                   # 第1章文件\n")
        f.write("    ├── 📹 chapter_2_*.*                   # 第2章文件\n")
        f.write("    └── ...                                # 其他章节文件\n")
        f.write("```\n")
    ...
    for i, chapter in enumerate(chapters, 1):
        ...
        chapters_info.append({
            ...
            'filename': f"chapter_{i}_{safe_filename(chapter['title'])}.md"
        })
    ...
    # 显示文件结构
    print(f"\n📁 文件总览:")
    print(f"   主报告文件: README.md")
    print(f"   章节报告: {len(chapters)} 个")
    print(f"   数据文件: chapters_info.json")
    print(f"   输出目录: {report_dir}")
