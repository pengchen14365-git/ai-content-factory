...
for ($i = 0; $i -lt $totalChapters; $i++) {
    ...
    Write-Host "  ✓ 剪辑视频完成" -ForegroundColor Green
    ...
    Write-Host "  ✓ 添加硬字幕完成" -ForegroundColor Green
...
"@
    ...
    Write-Host "  ✓ 生成总结文案完成" -ForegroundColor Green

    Write-Host "  ✅ 章节 $chapterIndex 处理完成!" -ForegroundColor Green
    Write-Host ""
}
...
Write-Host "总耗时: $((Get-Date) - $startTime).TotalSeconds 秒" -ForegroundColor Green
